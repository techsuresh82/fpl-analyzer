# FPL WEEKLY ANALYSIS REPORT
Generated: 2026-01-29 23:38:34
Manager: Suresh K
Team: Invincibles 2
Team ID: 8176746

## Team Status
- Team Value: £101.7m
- In Bank: £0.0m
- Free Transfers: 1

## ⚠️ PLAYERS TO CONSIDER TRANSFERRING OUT

### Heaven (DEF) - Man Utd - £3.8m
- Poor form (1.2)
- Low expected points (1.2)
- Next 3 fixtures: Fulham(H, FDR:2), Spurs(H, FDR:3), West Ham(A, FDR:2)

**Suggested replacements (max £3.8m):**
1. **Bornauw** (Leeds) - £3.8m
   - Form: 2.8, PPG: 2.3, xP: 1.8
   - Fixtures: Arsenal(H, FDR:4), Nott'm Forest(H, FDR:2), Chelsea(A, FDR:3)
   - Selected by: 0.1%
2. **Acheampong** (Chelsea) - £3.8m
   - Form: 1.0, PPG: 3.4, xP: 1.0
   - Fixtures: West Ham(H, FDR:2), Wolves(A, FDR:2), Leeds(H, FDR:2)
   - Selected by: 1.2%
3. **Clyne** (Crystal Palace) - £3.8m
   - Form: 1.6, PPG: 2.0, xP: 1.6
   - Fixtures: Nott'm Forest(A, FDR:3), Brighton(A, FDR:3), Burnley(H, FDR:2)
   - Selected by: 0.6%

---

### Acheampong (DEF) - Chelsea - £3.8m
- Poor form (1.0)
- Low expected points (1.0)
- Next 3 fixtures: West Ham(H, FDR:2), Wolves(A, FDR:2), Leeds(H, FDR:2)

**Suggested replacements (max £3.8m):**
1. **Bornauw** (Leeds) - £3.8m
   - Form: 2.8, PPG: 2.3, xP: 1.8
   - Fixtures: Arsenal(H, FDR:4), Nott'm Forest(H, FDR:2), Chelsea(A, FDR:3)
   - Selected by: 0.1%
2. **Acheampong** (Chelsea) - £3.8m
   - Form: 1.0, PPG: 3.4, xP: 1.0
   - Fixtures: West Ham(H, FDR:2), Wolves(A, FDR:2), Leeds(H, FDR:2)
   - Selected by: 1.2%
3. **Clyne** (Crystal Palace) - £3.8m
   - Form: 1.6, PPG: 2.0, xP: 1.6
   - Fixtures: Nott'm Forest(A, FDR:3), Brighton(A, FDR:3), Burnley(H, FDR:2)
   - Selected by: 0.6%

---

### Watkins (FWD) - Aston Villa - £8.8m
- **INJURED/UNAVAILABLE** - Hamstring injury - 75% chance of playing
- Next 3 fixtures: Brentford(H, FDR:3), Bournemouth(A, FDR:4), Brighton(H, FDR:3)

**Suggested replacements (max £8.8m):**
1. **Thiago** (Brentford) - £7.2m
   - Form: 7.0, PPG: 5.4, xP: 7.0
   - Fixtures: Aston Villa(A, FDR:3), Newcastle(A, FDR:4), Arsenal(H, FDR:4)
   - Selected by: 37.0%
2. **João Pedro** (Chelsea) - £7.2m
   - Form: 5.8, PPG: 4.5, xP: 5.8
   - Fixtures: West Ham(H, FDR:2), Wolves(A, FDR:2), Leeds(H, FDR:2)
   - Selected by: 31.2%
3. **Mané** (Wolves) - £4.5m
   - Form: 6.2, PPG: 3.2, xP: 6.2
   - Fixtures: Bournemouth(H, FDR:3), Chelsea(H, FDR:3), Nott'm Forest(A, FDR:3)
   - Selected by: 1.5%

---

## 🌟 YOUR TOP PERFORMERS

- **Rice** (MID) - Arsenal - £7.4m
  Form: 6.0, Expected: 7.0, Fixtures: Leeds(A, FDR:3), Sunderland(H, FDR:2), Brentford(A, FDR:3)
- **Wilson** (MID) - Fulham - £6.0m
  Form: 5.8, Expected: 5.8, Fixtures: Man Utd(A, FDR:3), Everton(H, FDR:2), Man City(A, FDR:4)
- **Mukiele** (DEF) - Sunderland - £4.5m
  Form: 5.6, Expected: 5.6, Fixtures: Burnley(H, FDR:2), Arsenal(A, FDR:5), Liverpool(H, FDR:4)
- **Chalobah** (DEF) - Chelsea - £5.6m
  Form: 4.8, Expected: 4.8, Fixtures: West Ham(H, FDR:2), Wolves(A, FDR:2), Leeds(H, FDR:2)
- **Semenyo** (MID) - Man City - £7.8m
  Form: 4.2, Expected: 4.7, Fixtures: Spurs(A, FDR:3), Liverpool(A, FDR:4), Fulham(H, FDR:2)

## 👑 CAPTAIN RECOMMENDATIONS

1. **Rice** - Expected: 7.0 points
   Next fixture: Leeds
2. **Wilson** - Expected: 5.8 points
   Next fixture: Man Utd
3. **Mukiele** - Expected: 5.6 points
   Next fixture: Burnley